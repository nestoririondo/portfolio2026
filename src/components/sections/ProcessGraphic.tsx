...
/** Bolder-earthy accent pop, layered on top of the themed sage/stone. */
const OCHRE = "#d8a24a";

/** Softer, pastel versions of the pops — used for the loose floating accents so
 *  they read like the pastel chips elsewhere on the site (no neon orbiting bits). */
const CLAY_SOFT = "color-mix(in oklab, #c06e4e 50%, #fff)";
const OCHRE_SOFT = "color-mix(in oklab, #d8a24a 48%, #fff)";

/** Hairline edge that matches the floating "Screen" shells (no dark ink outlines). */
const EDGE = "rgba(40,30,20,.10)";

/** A 4-point sparkle centered at the origin (radius ~6). */
const SPARKLE =
  "M0 -6 Q1.1 -1.1 6 0 Q1.1 1.1 0 6 Q-1.1 1.1 -6 0 Q-1.1 -1.1 0 -6 Z";
...
function FxLayer({ px, children }: { px: number; children: ReactNode }) {
  return (
    <div
      className="process-layer process-layer-bg"
      data-px={px}
      aria-hidden="true"
    >
  ...
  return (
    <>
      ...
      {kind === "talk" && (
        <>
          <FxLayer px={px * 1.5}>
            <circle cx="98" cy="26" r="9" fill={OCHRE_SOFT} />
          </FxLayer>
          <FxLayer px={px * -0.7}>
            <circle
              cx="20"
              ...
              r="8"
              fill="none"
              stroke="color-mix(in oklab, var(--accent) 45%, #fff)"
              strokeWidth="3"
            />
          </FxLayer>
        </>
        ...
        <>
          <FxLayer px={px * 1.4}>
            <rect
              ...
              rx="4"
              fill={OCHRE_SOFT}
              transform="rotate(14 100 30)"
            />
          </FxLayer>
          <FxLayer px={px * -0.6}>
            <circle cx="18" cy="90" r="7" fill="color-mix(in oklab, var(--accent) 45%, #fff)" />
          </FxLayer>
        </>
      )}
      ...
      {kind === "build" && (
        <>
          ...
          <FxLayer px={px * 1.8}>
            <path d={SPARKLE} fill={OCHRE_SOFT} transform="translate(102 24)" />
          </FxLayer>
          <FxLayer px={px * 0.5}>
            <path
              d={SPARKLE}
              fill="color-mix(in oklab, var(--accent) 45%, #fff)"
              transform="translate(19 34) scale(0.72)"
            />
          </FxLayer>
          <FxLayer px={px * -0.9}>
            <circle cx="14" cy="66" r="3" fill={CLAY_SOFT} />
          </FxLayer>
          <FxLayer px={px * 1.2}>
            <rect
              x="94"
          ...
          <FxLayer px={px * -0.4}>
            <rect
              ...
              rx="2"
              fill={OCHRE_SOFT}
              transform="rotate(-18 21 91)"
            />
          </FxLayer>
          <FxLayer px={px * 1.5}>
            <g stroke={OCHRE_SOFT} strokeWidth="2.6" strokeLinecap="round">
          ...
          {kind === "plan" && (
            <>
              ...
              <g transform="translate(62 60)">
                <path
                  d="M0 0 L0 24 L6 18 L10.5 27 L13.5 25.5 L9 16.5 L16.5 16.5 Z"
                  fill="var(--ink)"
                  stroke="var(--paper)"
                  strokeWidth="1.8"
                  strokeLinejoin="round"
          ...
          {kind === "build" && (
            <>
              ...
              <rect
                ...
                height="7"
                rx="2.5"
                fill="color-mix(in oklab, #d8a24a 78%, #fff)"
              />
              <rect
                x="62"
                y="56"
                ...
                height="7"
                rx="2.5"
                fill="color-mix(in oklab, #c06e4e 78%, #fff)"
              />
              <rect
                x="44"
                y="68"
